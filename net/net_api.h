...
namespace net_com
{
	...
	void SendGetNodeCacheReq(const Node& dest, bool is_fetch_public);
	void RegisterToPublic();//liuzg
	void DealHeart();
...
bool net_com::broadcast_message(T& msg, const net_com::Compress isCompress, const net_com::Encrypt isEncrypt, const net_com::Priority priority)
{
	...
	std::string defaultBs58Addr = Singleton<accountinfo>::get_instance()->DefaultKeyBs58Addr;
	...
	if (selfNode.is_public_node)
	{
		...
		req.mutable_from()->set_base58addr(selfNode.base58address);
	...
	{
		net_com::send_message(selfNode.public_base58addr, req, isCompress, isEncrypt, priority );
	}
