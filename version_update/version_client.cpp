...
bool Version_Client::verify_data()
{
    ...
    if (m_hash == hash)
    {   
        DEBUGLOG("Data verification is successful and data is complete.");
        return true;
    }
    else
    {
        DEBUGLOG("Data validation failed. Data is incorrect.");
        return false;
    }  
}
...
{
    DEBUGLOG("Start downloading the latest version...");
    cout << "Start downloading the latest version..." << endl;
    ...
    DEBUGLOG("The latest version is downloaded...");
    cout << "The latest version is downloaded..." << endl;
    return true;
}
...
{
    DEBUGLOG("Prepare the setup and start the update script...");
    if(Singleton<Config>::get_instance()->UpdateConfig(1, m_target_version, m_version_server_ip))
    {
        cout << "After the configuration file is updated, decompress the file!" << endl;
