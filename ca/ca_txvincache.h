...
public:
    ...
    // 添加交易
    int Add(const CTransaction& tx, uint64_t prevBlkHeight, bool broadcast = true);
...
private:
    ...
    int Add(const string& hash, const vector<string>& vin, const vector<string>& fromAddr, const vector<string>& toAddr, uint64_t amount, uint64_t prevBlkHeight);

    bool VerifySign(const CTransaction& tx);
    void ConvertTx(const CTransaction& tx, uint64_t prevBlkHeight, TxVinCache::Tx& newTx);
    void BroadcastTxPending(const CTransaction& tx, uint64_t prevBlkHeight);
