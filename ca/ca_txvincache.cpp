...
int TxVinCache::Add(const CTransaction& tx, uint64_t prevBlkHeight,  bool broadcast/* = true*/)
{
    if (!VerifySign(tx))
    {
    ...
    TxVinCache::Tx newTx;
    ConvertTx(tx, prevBlkHeight, newTx);

    int result = Add(newTx);
    if (result == 0 && broadcast)
    {
        BroadcastTxPending(tx, prevBlkHeight);
    }
...
int TxVinCache::Add(const string& hash, const vector<string>& vin, const vector<string>& fromAddr, const vector<string>& toAddr, uint64_t amount, uint64_t prevBlkHeight)
{
    Tx tx;
    tx.txHash = hash;
    tx.vins = vin;
    ...
    tx.prevBlkHeight = prevBlkHeight;
...
void TxVinCache::ConvertTx(const CTransaction& tx, uint64_t prevBlkHeight, TxVinCache::Tx& newTx)
{
    ...
    if (txType == TXTYPE_REDEEM)
    {
        ...
        std::string strTxRaw;
        DBReader db_resder;
        if (DBStatus::DB_SUCCESS == db_resder.GetTransactionByHash(utxoStr, strTxRaw))
        {
            CTransaction utxoTx;
...
void TxVinCache::BroadcastTxPending(const CTransaction& tx, uint64_t prevBlkHeight)
{
    ...
    txPendingMsg.set_prevblkheight(prevBlkHeight);
...
void TxVinCache::DoClearExpire()
{
    ...
    for (auto iter = txs_.begin(); iter != txs_.end();)
    {
        ...
        {
            SendConfirmTransaction(iter->txHash, iter->prevBlkHeight, ConfirmTxFlag);
