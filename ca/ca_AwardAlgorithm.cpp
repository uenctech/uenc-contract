...
int AwardAlgorithm::Build(uint64_t top,
                        uint need_verify_count, 
                        ...
                        const std::vector<uint64_t> &vec_sign_sum) 
{
    ...
    uint64_t blockNum = 0;
    if (0 != GetBlockNumInUnitTime(top, blockNum) )
    {
...
int AwardAlgorithm::GetAward(const uint64_t blockNum, uint64_t & awardAmount)
{
    ...
    if (DBStatus::DB_SUCCESS != db_reader.GetBlockHashByBlockHeight(0, blockHahs) )
    {
    ...
    /// TODO
    uint64_t award = 0;
    if (DBStatus::DB_SUCCESS != db_reader.GetAwardTotal(award) )
    {
...
int AwardAlgorithm::GetTimeFromBlockHash(const std::string & blockHash, uint64_t & blockTime)
{
    DBReader db_reader;
    std::string strBlock;
    if (DBStatus::DB_SUCCESS != db_reader.GetBlockByBlockHash(blockHash, strBlock) )
    {
...
int AwardAlgorithm::GetBlockNumInUnitTime(uint64_t top, uint64_t & blockSum)
{
    ...
    if (DBStatus::DB_SUCCESS != db_read.GetBlockHashsByBlockHeight(top, blockHashs) )
    {
...
int AwardAlgorithm::GetSignCountByAddr(const std::string addr, uint64_t & count) 
{
    ...
    if (DBStatus::DB_SUCCESS != db_reader.GetAwardCount(count) )
    {
...
int AwardAlgorithm::GetAwardAmountByAddr(const std::string addr, uint64_t & awardAmount) 
{
    ...
    for (auto blockHash : blockHashs) 
    {
        ...
        std::string rawBlock;
        if (DBStatus::DB_SUCCESS != db_reader.GetBlockByBlockHash(blockHash, rawBlock) )
        {
