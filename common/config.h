...
public:
    Config(){if(!this->InitFile()){_Exit(0);}};
    ~Config(){};
    //根据文件名构造配置文件
  ...
  //  bool InitPWDFile(const std::string &name = "devpwd.json");
   // bool NewDevPWDFile(std::string strFile);
    ...
    void SetIsPublicNode(bool isPublic);
  ...
  /**
   ...
   Type  GetProxyTypeStatus();
    ...
    void SetSyncDataCount(unsigned int data);
    /**
    ...
    void SetSyncDataPollTime(unsigned int pollTime);
    ...
    std::string GetHttpCallbackPath();
