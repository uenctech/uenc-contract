...
namespace wasm {
    ...
    wasm_vm_runtime<Impl>::instantiate_module(const char *code_bytes, size_t code_size) {
        ...
        try {
            ...
            CHAIN_THROW(wasm_chain::wasm_execution_error, "11111111Error building eos-vm interp: %s", e.what());
        }
    }
    ...
    wasm_vm_runtime2<Impl>::instantiate_module(const char *code_bytes, size_t code_size, const uint256 &hash) {
        ...
        try {
            ...
            CHAIN_THROW(wasm_chain::wasm_execution_error, "22222222222Error building eos-vm interp: %s", e.what());
        }
    }
