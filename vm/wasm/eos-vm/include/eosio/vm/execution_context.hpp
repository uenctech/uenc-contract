    ...
    public:
      ...
      inline void call(uint32_t index) {
         ...
         if (index < _mod.get_imported_functions_size()) 
         {
          ...
          //  std::cout <<" pop_call---------------------pop_call"<< std::endl;
            pop_call();
         } 
         else 
         {
      ...
      inline operand_stack& get_operand_stack() 
      {
         return _os;
      ...
      { 
         ...
         //std::cout << "_mod.tables[0].table[i]:"<< _mod.tables[0].table[i] <<std::endl;
      }
      inline void push_operand(operand_stack_elem el) 
      { 
         _os.push(std::move(el)); 
         //std::cout << "push_operand" <<std::endl;
         }
      inline operand_stack_elem get_operand(uint16_t index) const 
      { 
         //std::cout << "operand_stack_elem get_operand:"  <<std::endl;
         return _os.get(_last_op_index + index); 
      }
      inline void           eat_operands(uint16_t index) 
      { 
         _os.eat(index); 
        // std::cout << "eat_operands:"  <<std::endl;
      }
      inline void           compact_operand(uint16_t index) 
      { 
         _os.compact(index); 
        // std::cout << "_os:"  <<std::endl;
      }
      ...
      { 
        ...
        // std::cout << "compact_operand"  <<std::endl;
      }
      inline uint16_t       current_operands_index() const 
      { 
        // std::cout << "current_operands_index"  <<std::endl;
         return _os.current_index(); 
      }
      inline void           push_call(activation_frame&& el) 
      { 
         //std::cout << "push_call"  <<std::endl;
         _as.push(std::move(el)); 
      }
      inline activation_frame pop_call() 
      { 
        // std::cout << "pop_call"  <<std::endl;
         return _as.pop();
      }
      inline uint32_t       call_depth()const 
      { 
         //std::cout << "call_depth"  <<std::endl;
         return _as.size();
      }
      template <bool Should_Exit=false>
      inline void           push_call(uint32_t index) 
      {
         //std::cout << "inline void           push_call(uint32_t index) "  <<std::endl;
      ...
      {
        // std::cout << "apply_pop_call(uint32_t num_locals, uint16_t return_count):"  <<std::endl;
      ...
      { 
         //std::cout << "operand_stack_elem  pop_operand:"  <<std::endl;
         return _os.pop(); 
      }
      inline operand_stack_elem& peek_operand(size_t i = 0) 
      { 
         //std::cout << "peek_operand"  <<std::endl;
         return _os.peek(i); 
      }
